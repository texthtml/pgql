...
type Resolver = for<'a> fn(&'a juniper::Executor<crate::context::Context, juniper::DefaultScalarValue>) -> ReturnType<'a>;

trait Registrable: Send + Sync
{
    fn register<'r>(self: &Self, registry: &mut juniper::Registry<'r>) -> juniper::meta::Field<'r, juniper::DefaultScalarValue>;
    fn name(self: &Self) -> String;
    fn resolve<'a>(self: &Self, executor: &'a juniper::Executor<crate::context::Context, juniper::DefaultScalarValue>) -> ReturnType<'a>;
}
...
impl juniper::GraphQLType<juniper::DefaultScalarValue> for Query {
    ...
    fn name(info: &Self::TypeInfo) -> Option<&str> { Some(&info.name) }
...
pub fn build(_config: &crate::Config) -> Schema {
    let fields: Vec<Box<dyn Registrable>> = vec![
        Box::new(FieldInfo::<i32> {
